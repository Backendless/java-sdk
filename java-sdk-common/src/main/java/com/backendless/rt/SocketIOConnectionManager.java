...
abstract class SocketIOConnectionManager
{
  ...
  Socket get()
  {
    ...
    synchronized( lock )
    {
      ...
      opts.path = "/" + injector.getPrefs().getApplicationIdOrDomain();

      opts.query = "apiKey=" + injector.getPrefs().getApiKey() + "&clientId=" + injector.getPrefs().getDeviceId() + "&binary=true";

      final String host = rtLookupService.lookup() + opts.path;
      logger.info( "Looked up for server " + host );

      String userToken = injector.getHeadersManager().getHeader( IHeadersManager.HeadersEnum.USER_TOKEN_KEY );
