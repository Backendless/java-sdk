...
public final class UserService
{
  ...
  public BackendlessUser CurrentUser( boolean reload )
  {
    if( reload )
    {
      ...
      {
        currentUser = injector.getUserService().findById( currentUser.getObjectId() );
        return currentUser;
      }
      else
      {
  ...
  public void CurrentUser( boolean reload, final AsyncCallback<BackendlessUser> responder )
  {
    if( reload )
    {
      if( currentUser != null && currentUser.getObjectId() != null && !currentUser.getObjectId().isEmpty() )
      {
        findById( currentUser.getObjectId(), new AsyncCallback<BackendlessUser>()
        {
          @Override
  ...
  public BackendlessUser login( final String login, final String password, boolean stayLoggedIn ) throws BackendlessException
  {
  ...
  public BackendlessUser login( final String objectId, boolean stayLoggedIn ) throws BackendlessException
  {
    synchronized( currentUserLock )
    {
      if( currentUser != null && !currentUser.isEmpty() )
  ...
  public void loginWithOAuth2( String authProviderCode, String accessToken, final Map<String, String> fieldsMappings,
  ...
  public void loginWithOAuth2( String authProviderCode, String accessToken, BackendlessUser guestUser,
                               final Map<String, String> fieldsMappings, final AsyncCallback<BackendlessUser> responder,
  ...
  public void loginWithOAuth1( String authProviderCode, String authToken, String authTokenSecret, Map<String, String> fieldsMappings,
  ...
  public void loginWithOAuth1( String authProviderCode, String authToken, String authTokenSecret, BackendlessUser guestUser,
                               Map<String, String> fieldsMappings, AsyncCallback<BackendlessUser> responder, boolean stayLoggedIn )
  {
    AsyncCallback<BackendlessUser> internalResponder = getUserLoginAsyncHandler( responder, stayLoggedIn );
  ...
  private void handleLogout()
  {
    ...
    injector.getHeadersManager().removeHeader( IHeadersManager.HeadersEnum.USER_TOKEN_KEY );
    injector.getUserTokenStorage().set( "" );
    injector.getUserIdStorage().set( "" );
    injector.getRtClient().userLoggedOut();
  }
  ...
  public boolean verifyPassword( String password )
  {
    ...
    String userToken = injector.getUserTokenStorage().get();
    if( userToken == null || userToken.isEmpty() )
      throw new IllegalArgumentException( ExceptionMessage.NO_EXISTING_USER_TOKEN );

    injector.getHeadersManager().addHeader( IHeadersManager.HeadersEnum.USER_TOKEN_KEY, userToken );
  ...
  public void findByRole( String roleName, boolean loadRoles, DataQueryBuilder queryBuilder,
                          final AsyncCallback<List<BackendlessUser>> responder )
  {
    try
    {
      if( roleName != null && roleName.isEmpty() )
      ...
      Invoker.invokeAsync( USER_MANAGER_SERVER_ALIAS, "findByRole", new Object[] { roleName, loadRoles, queryBuilder.build() }, responder, ResponderHelper.getCollectionAdaptingResponder( BackendlessUser.class ) );
    }
    catch( Throwable e )
    {
      if( responder != null )
  ...
  public void getAuthorizationUrlLink( String authProviderCode, Map<String, String> fieldsMappings, List<String> scope,
                                       AsyncCallback<String> responder ) throws BackendlessException
  {
    ...
    String origin = null;
    String deviceType = injector.getHeadersManager().getHeader( IHeadersManager.HeadersEnum.APP_TYPE_NAME );
  ...
  public void getAuthorizationUrlLink( String authProviderCode, String callbackUrlDomain, Map<String, String> fieldsMappings,
                                       List<String> scope, AsyncCallback<String> responder ) throws BackendlessException
  {
    if( fieldsMappings == null )
      fieldsMappings = new HashMap<>();
  ...
  {
    return injector.getUserIdStorage().get();
  }
  ...
  private void handleUserLogin( BackendlessUser invokeResult, boolean stayLoggedIn )
  {
    ...
    if( stayLoggedIn )
    {
      injector.getUserTokenStorage().set( userToken );
      injector.getUserIdStorage().set( this.CurrentUser().getUserId() );
    }
    else
    {
      injector.getUserTokenStorage().set( "" );
    ...
    injector.getRtClient().userLoggedIn( userToken );
  }

  private AsyncCallback<BackendlessUser> getUserLoginAsyncHandler( final AsyncCallback<BackendlessUser> responder,
